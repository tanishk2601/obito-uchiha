...
from Obito_Probot import REDLIONS
from Obito_Probot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def obito_is_admin(chat_id: int):
    status = False
    obito = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
